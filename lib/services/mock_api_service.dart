...
class MockApiService implements IApiService {
  ...
  Future<dynamic> call(String ipAddress, String sysauth, bool useHttps, {required String object, required String method, Map<String, dynamic>? params, BuildContext? context}) async {
    ...
    try {
      ...
      if (mockDataFile != null) {
        try {
          final jsonString = await rootBundle.loadString('${AppConfig.mockDataPath}$mockDataFile');
          final jsonData = jsonDecode(jsonString);
          return [0, jsonData]; // Wrap in standard RPC response format
        } catch (e) {
          // Log file loading error and fall back to default data
          debugPrint('MockApiService: Failed to load mock data file "$mockDataFile" for endpoint "$endpointKey": $e');
      ...
      if (defaultData[1] is Map && (defaultData[1] as Map).isEmpty) {
        debugPrint('MockApiService: No mock data available for endpoint "$endpointKey", returning empty response');
      }
      return defaultData;
      
    } catch (e) {
      // Catch-all error handler
      debugPrint('MockApiService: Unexpected error for endpoint "$endpointKey": $e');
  ...
  Future<Map<String, Set<String>>> fetchAssociatedStations() async {
    ...
    try {
      final jsonString = await rootBundle.loadString('${AppConfig.mockDataPath}associated_stations.json');
      final jsonData = jsonDecode(jsonString);
      ...
      return {
        'wlan0': {
          ...
          'bb:cc:dd:11:22:33', 'bb:cc:dd:44:55:66', 'bb:cc:dd:77:88:99'
        },
        'wlan1': {
          'aa:bb:cc:aa:bb:cc', 'aa:bb:cc:dd:ee:ff', 'aa:bb:cc:12:34:56',
          'bb:cc:dd:aa:bb:cc', 'bb:cc:dd:dd:ee:ff', 'aa:bb:cc:65:43:21'
        },
      };
    }
  }
  ...
  Future<Map<String, dynamic>?> fetchWireGuardPeers({required String ipAddress, required String sysauth, required bool useHttps, required String interface, BuildContext? context}) async {
    ...
    try {
      final jsonString = await rootBundle.loadString('${AppConfig.mockDataPath}wireguard_peers.json');
      final jsonData = jsonDecode(jsonString);
        ...
        interface: {
          ...
          'peers': {
            'peer_public_key_1': {
              ...
              'last_handshake': _getVariedTimestamp() - _random.nextInt(300) - 30,
            ...
            'peer_public_key_2': {
              ...
              'last_handshake': _getVariedTimestamp() - _random.nextInt(600) - 60,
  ...
  static Map<String, int> _getVariedMemory() {
    ...
    final int freeMemory = 134217728 - usedVariation; // Base 128MB minus variation
    ...
    return {
      ...
      'available': freeMemory + 20971520 + _random.nextInt(10485760), // Free + some cache
    };
  }

  static Map<String, int> _getVariedNetworkStats(String device) {
    final now = DateTime.now().millisecondsSinceEpoch;
    final seconds = (now ~/ 1000) % 3600; // Reset every hour for predictable patterns
    
    if (device == 'eth0') {
      // Create realistic throughput patterns with sine waves and random noise
      final timeBasedMultiplier = 1.0 + 0.5 * sin(seconds * 2 * pi / 300); // 5-minute cycles
      final burstMultiplier = _random.nextBool() ? (1.0 + _random.nextDouble() * 2) : 1.0; // Random bursts
      
      // Base rates: 50-500 KB/s for download, 10-100 KB/s for upload  
      final rxIncrement = ((50000 + _random.nextInt(450000)) * timeBasedMultiplier * burstMultiplier).round();
      final txIncrement = ((10000 + _random.nextInt(90000)) * timeBasedMultiplier * burstMultiplier * 0.3).round();
      
      _baseRxBytes += rxIncrement;
      _baseTxBytes += txIncrement;
      _baseRxPackets += (rxIncrement / 1500).round() + _random.nextInt(50); // ~1500 bytes per packet
      ...
      final timeBasedMultiplier = 1.0 + 0.3 * sin(seconds * 2 * pi / 180); // 3-minute cycles
      final localActivity = _random.nextBool() ? (1.0 + _random.nextDouble()) : 0.5;
      
      // LAN typically has lower but more consistent throughput
      final rxIncrement = ((20000 + _random.nextInt(100000)) * timeBasedMultiplier * localActivity).round();
      final txIncrement = ((15000 + _random.nextInt(80000)) * timeBasedMultiplier * localActivity).round();
  ...
  static String _getVariedDhcpLeases() {
    ...
    final devices = [
      'iPhone-John', 'MacBook-Pro', 'Smart-TV-Living-Room', 'Gaming-PC', 
      'Nest-Thermostat', 'iPad-Sarah', 'Amazon-Echo', 'Samsung-Galaxy-S23',
      'Dell-Laptop-Work', 'Ring-Doorbell', 'Nintendo-Switch', 'Philips-Hue-Bridge'
    ];
    final macAddresses = [
      'aa:bb:cc:11:22:33', 'aa:bb:cc:44:55:66', 'aa:bb:cc:77:88:99', 
      'aa:bb:cc:aa:bb:cc', 'aa:bb:cc:dd:ee:ff', 'aa:bb:cc:12:34:56',
      'aa:bb:cc:65:43:21', 'bb:cc:dd:11:22:33', 'bb:cc:dd:44:55:66',
      'bb:cc:dd:77:88:99', 'bb:cc:dd:aa:bb:cc', 'bb:cc:dd:dd:ee:ff'
    ];
    final ipAddresses = [
    ...
    for (int i = 0; i < devices.length; i++) {
      ...
      leases += '$leaseTime ${macAddresses[i]} ${ipAddresses[i]} ${devices[i]} 01:${macAddresses[i]}\n';
    }
  ...
  @override
  Future<List<String>> fetchAssociatedStationsWithContext({required String ipAddress, required String sysauth, required bool useHttps, required String interface, BuildContext? context}) async {
    await Future.delayed(const Duration(milliseconds: 200));
  ...
  @override
  Future<Map<String, Set<String>>> fetchAllAssociatedWirelessMacsWithContext({required String ipAddress, required String sysauth, required bool useHttps, BuildContext? context}) async {
